...
/**
 * Server-side environment schema for Auth.js.
 *
 * At least one authentication method must be configured:
 * - GitHub OAuth (AUTH_GITHUB_ID + AUTH_GITHUB_SECRET)
 * - Google OAuth (AUTH_GOOGLE_ID + AUTH_GOOGLE_SECRET)
 * - Email + Password (AUTH_EMAIL_ENABLED=true)
 */
  ...
  .superRefine((value, ctx) => {
    ...
    if (hasGitHubId !== hasGitHubSecret) {
      ctx.addIssue({
        code: "custom",
        message:
          "AUTH_GITHUB_ID and AUTH_GITHUB_SECRET must both be set or both be omitted",
        path: hasGitHubId ? ["AUTH_GITHUB_SECRET"] : ["AUTH_GITHUB_ID"],
      });
    }
    ...
    const hasGitHub = hasGitHubId && hasGitHubSecret;
    const hasGoogle = hasGoogleId && hasGoogleSecret;
    const hasEmail = value.AUTH_EMAIL_ENABLED === "true";

    if (!hasGitHub && !hasGoogle && !hasEmail) {
      ctx.addIssue({
        code: "custom",
        message:
          "Configure at least one auth method: GitHub, Google, or AUTH_EMAIL_ENABLED=true",
...
export function parseAuthEnv(env: Record<string, string | undefined>): AuthEnv {
  return authEnvSchema.parse({
    ...
    AUTH_GITHUB_ID: env.AUTH_GITHUB_ID || undefined,
    AUTH_GITHUB_SECRET: env.AUTH_GITHUB_SECRET || undefined,
    AUTH_GOOGLE_ID: env.AUTH_GOOGLE_ID || undefined,
